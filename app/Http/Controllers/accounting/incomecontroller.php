...
class incomecontroller extends Controller
{
    ...
    public function index()
    {
        ...
        // ✅ القائمة المنسدلة — Scope يعمل تلقائياً
        $BranchesList = Branch::where('status', 1)->orderByDesc('id')->get();

        $IncomeTypesList = IncomeType::where('status', 1)->orderByDesc('id')->get();

        // ✅ إيرادات فروع المستخدم فقط
        $Incomes = Income::with([
            'type',
    ...
    public function create() { return redirect()->route('income.index'); }
    public function show($id) { return redirect()->route('income.index'); }
    public function edit($id) { return redirect()->route('income.index'); }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($isCancelled) {
            ...
            if (empty($row->cancelledat))  $row->cancelledat = Carbon::now();
            if (empty($row->usercancel))   $row->usercancel  = Auth::id();
        } else {
            $row->iscancelled  = false;
            $row->usercancel   = null;
    ...
    public function employees_by_branch(Request $request)
    {
        try {
            ...
            return response()->json(['ok' => false, 'message' => trans('accounting.ajax_error_try_again')], 500);
        }
    }
}
