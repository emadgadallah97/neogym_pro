...
class payrollscontroller extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        $created = $updated = $skippedLocked = 0;
        ...
        try {
            ...
            foreach ($employees as $emp) {
                ...
                $attendancePreview = $this->computeAttendanceDeductionsPreview(
                    ...
                    (float)$payroll->base_salary, $monthStart
                );
            ...
            DB::rollBack();
            Log::error('payrolls.generate error', ['msg' => $e->getMessage(), 'file' => $e->getFile(), 'line' => $e->getLine()]);
    ...
    public function cancelDrafts(Request $request)
    {
        ...
        try {
            ...
            if ($draftPayrolls->count() === 0) {
                DB::commit();
                return response()->json(['success' => false, 'message' => trans('hr.no_data') ?? 'لا يوجد مسودات للحذف'], 422);
            }
    ...
    public function approveMonth(Request $request)
    {
        ...
        try {
            ...
            if ($payrolls->count() === 0) {
                DB::commit();
                return response()->json(['success' => false, 'message' => trans('hr.no_data_to_approve') ?? 'لا يوجد رواتب مسودة للاعتماد'], 422);
            }
            ...
            foreach ($payrolls as $p) {
                ...
                $approvedQuery = fn($q) => $q->where('status', 'approved')
                    ->orWhere('status', 1)->orWhere('status', '1');

                HrOvertime::where('employee_id', $empId)->where('branch_id', $branchId)
                    ->whereYear('applied_month', $monthStart->year)->whereMonth('applied_month', $monthStart->month)
                    ->where($approvedQuery)->whereNull('payroll_id')
                    ->update(['status' => 'applied', 'payroll_id' => $p->id]);

                HrAllowance::where('employee_id', $empId)->where('branch_id', $branchId)
                    ->whereYear('applied_month', $monthStart->year)->whereMonth('applied_month', $monthStart->month)
                    ->where($approvedQuery)->whereNull('payroll_id')
                    ->update(['status' => 'applied', 'payroll_id' => $p->id]);

                HrDeduction::where('employee_id', $empId)->where('branch_id', $branchId)
                    ->whereYear('applied_month', $monthStart->year)->whereMonth('applied_month', $monthStart->month)
                    ->where($approvedQuery)->whereNull('payroll_id')
                    ->update(['status' => 'applied', 'payroll_id' => $p->id]);
                ...
                HrAdvanceInstallment::where('employee_id', $empId)
                    ->whereDate('month', $monthStart->toDateString())
                    ->where('is_paid', false)->whereNull('payroll_id')
    ...
    public function payMonth(Request $request)
    {
        ...
        try {
            ...
            if ($payrolls->count() === 0) {
                DB::commit();
                return response()->json(['success' => false, 'message' => trans('hr.no_data_to_pay') ?? 'لا يوجد رواتب معتمدة للصرف'], 422);
            }
            ...
            HrAdvanceInstallment::whereIn('payroll_id', $payrollIds)
                ->where('is_paid', false)
                ->update(['is_paid' => true, 'paid_date' => $paymentDate]);

            if ($instAgg->count() > 0) {
                $advances = HrAdvance::whereIn('id', $instAgg->keys()->toArray())->lockForUpdate()->get();
                ...
                foreach ($advances as $adv) {
                    ...
                    $remain = max(0, round((float)($adv->remaining_amount ?? 0) - $add, 2));
            ...
            return response()->json([
                ...
                'data'    => ['paid_rows' => $payrolls->count(), 'payment_date' => $paymentDate],
            ]);
        } catch (\Throwable $e) {
            DB::rollBack();
    ...
    public function breakdown(Request $request)
    {
        ...
        // ── closure مشتركة للحالة المعتمدة ──
        $approvedOrLinked = function ($q) use ($payrollId) {
            $q->where(function ($q2) {
            ...
            if ($payrollId) {
                $q->orWhere('payroll_id', $payrollId);
            }
        };
        ...
        foreach ($att as $a) {
            ...
            match ($s) {
                'late'     => ($attKpi['late_days']++    && $attKpi['present_days']++),
                'present'  => $attKpi['present_days']++,
                'absent'   => $attKpi['absent_days']++,
                'halfday', 'half_day' => $attKpi['halfday_days']++,
                default    => null,
            };
        }
    ...
    private function computeMonthAmounts(int $employeeId, int $branchId, Carbon $monthStart): array
    {
        ...
        $overtime = (float) HrOvertime::where('employee_id', $employeeId)
            ...
            ->whereYear('applied_month', $monthStart->year)
            ->whereMonth('applied_month', $monthStart->month)
            ->where($approved)->whereNull('payroll_id')
            ->sum('total_amount');
    ...
    private function autoGenerateOvertimeFromAttendance(int $branchId, string $dateFrom, string $dateTo, $employees): array
    {
        ...
        $created        = $skippedExists = $skippedNoShift = $skippedNoTimes = $skippedZero = 0;
        ...
        foreach ($attRows as $att) {
            ...
            $exists = HrOvertime::where('employee_id', $empId)
                ->where('branch_id', $branchId)
                ->where(fn($q) => $q->whereDate('date', $attDate)->orWhere('attendance_id', $att->id))
                ->exists();
            ...
            if (!array_key_exists($cacheKey, $shiftCache)) {
                $shiftCache[$cacheKey] = HrEmployeeShift::with('shift')
                    ->where('employee_id', $empId)
                    ->where('branch_id', $branchId)
                    ->where('status', 1)
                    ->where(fn($q) => $q->whereNull('start_date')->orWhereDate('start_date', '<=', $attDate))
            ...
            if (!$shift || !$shift->start_time || !$shift->end_time) { $skippedNoShift++; continue; }

            $inTime  = $this->extractTimeOnly($att->getRawOriginal('check_in')  ?? $att->check_in);
            $outTime = $this->extractTimeOnly($att->getRawOriginal('check_out') ?? $att->check_out);

            if (!$inTime || !$outTime) { $skippedNoTimes++; continue; }
    ...
    private function extractTimeOnly($v): ?string
    {
        ...
        try {
            if ($v instanceof \DateTimeInterface) return Carbon::instance($v)->format('H:i:s');
            $s = (string)$v;
            if (preg_match('/\d{4}-\d{2}-\d{2}/', $s)) return Carbon::parse($s)->format('H:i:s');
