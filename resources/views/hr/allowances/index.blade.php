...
@section('title')
    {{ trans('hr.allowances') ?? 'المكافآت والبدلات' }} | {{ trans('main_trans.title') }}
@endsection
    ...
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font">
                ...
                {{ trans('hr.allowances') ?? 'المكافآت والبدلات' }}
            </h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="{{ route('hr.index') }}">{{ trans('hr.title') ?? 'الموارد البشرية' }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('hr.allowances') ?? 'المكافآت والبدلات' }}</li>
                </ol>
            </div>
        </div>
    </div>
...
<form method="GET" action="{{ route('allowances.index') }}" class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-2">
                ...
                <select name="type" id="filter_type" class="form-select font">
                    <option value="">{{ trans('hr.all_types') ?? 'كل الأنواع' }}</option>
                    @foreach($types as $k => $v)
                        <option value="{{ $k }}" {{ $typeFilter===$k ? 'selected' : '' }}>{{ trans('hr.allowance_type_'.$k) ?? $v }}</option>
                    @endforeach
                </select>
            </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header">
        <h5 class="card-title mb-0 font">{{ trans('hr.allowances_list') ?? 'قائمة المكافآت والبدلات' }}</h5>
    </div>

    <div class="card-body">
        <table id="allowancesTable" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
            <thead>
                <tr>
                    <th>#</th>
            ...
            <tbody>
                ...
                @foreach($rows as $r)
                    ...
                    <tr id="row-{{ $r->id }}" data-id="{{ $r->id }}">
                        ...
                        <td class="text-center"><span class="badge bg-info text-dark">{{ trans('hr.allowance_type_'.$r->type) ?? ($types[$r->type] ?? $r->type) }}</span></td>
...
{{-- Modal --}}
<div id="allowanceModal" class="modal fade" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 overflow-hidden">
            <div class="modal-header p-3">
                <h4 class="card-title mb-0 font" id="modalTitle">{{ trans('hr.add_allowance') ?? 'إضافة مكافأة/بدل' }}</h4>
            ...
            <div class="modal-body">
                ...
                <form id="allowanceForm" autocomplete="off">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="type" id="form_type" class="form-select font" required>
                                @foreach($types as $k => $v)
                                    <option value="{{ $k }}">{{ trans('hr.allowance_type_'.$k) ?? $v }}</option>
                                @endforeach
                            </select>
...
$(document).ready(function () {
    ...
    var table = $('#allowancesTable').DataTable({
        responsive: true,
    ...
    function initSelect2(){
        ...
        $('#filter_type').select2({ width:'100%' });

        $('#form_branch_id').select2({ width:'100%', dropdownParent: $('#allowanceModal') });
        $('#form_employee_id').select2({ width:'100%', dropdownParent: $('#allowanceModal') });
        $('#form_type').select2({ width:'100%', dropdownParent: $('#allowanceModal') });
    }
    initSelect2();
    ...
    $('#filter_branch_id').on('change', function(){
        ...
        $.get('{{ route('allowances.employees.byBranch') }}', { branch_id: branchId }, function(res){
            if (res.success) {
    ...
    $('#btnOpenAdd').on('click', function(){
        clearErrors();
        $('#modalTitle').text('{{ trans('hr.add_allowance') ?? 'إضافة مكافأة/بدل' }}');
        $('#form_id').val('');
        ...
        $('#form_type').val('bonus').trigger('change');
    ...
    $(document).on('click', '.btn-edit', function(){
        ...
        $.get('{{ url('allowances') }}/' + id, function(res){
            if(!res.success){
            ...
            $('#modalTitle').text('{{ trans('hr.edit_allowance') ?? 'تعديل مكافأة/بدل' }}');
            $('#form_id').val(d.id);
    ...
    $('#allowanceForm').on('submit', function(e){
        ...
        var id = $('#form_id').val();
        var url = id ? ('{{ url('allowances') }}/' + id) : '{{ route('allowances.store') }}';
        ...
        $.ajax({
            ...
            success: function(res){
                ...
                var typeLabel = '{{ trans('hr.allowance_type_bonus') ?? 'مكافأة' }}';
                var typeMap = @json($types);
                if (typeMap && typeMap[d.type]) typeLabel = typeMap[d.type];

                var rowData = [
                    '',
                    d.employee_name + ' <small class="text-muted">(' + (d.employee_code ?? '') + ')</small>',
                    d.branch_name,
                    '<div class="text-center"><span class="badge bg-info text-dark">' + (typeLabel) + '</span></div>',
    ...
    $(document).on('click', '.btn-approve', function(){
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                text: '{{ trans('hr.allowance_approve_confirm_msg') ?? 'هل تريد الاعتماد؟' }}',
                icon: 'question',
