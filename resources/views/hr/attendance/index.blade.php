...
@section('content')
    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
            <table id="attendanceTable" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                style="width:100%">
                <thead>
                    <tr>
                        <th>#</th>
                ...
                <tbody>
                    ...
                    @foreach($rows as $r)
                        @php $i++; @endphp
                        <tr id="{{ $r['row_dom_id'] }}" data-virtual="{{ $r['is_virtual'] }}"
                            data-employee="{{ $r['employee_id'] }}" data-date="{{ $r['date'] }}"
                                ...
                                <div class="d-flex justify-content-center gap-1">
                                    ...
                                    @if(!$r['is_virtual'] && $r['attendance_id'])
                                        <button type="button" class="btn btn-soft-danger btn-sm btn-delete"
                                            data-id="{{ $r['attendance_id'] }}" data-employee="{{ $r['employee_id'] }}"
                                            data-date="{{ $r['date'] }}" data-name="{{ $r['employee_name'] }}">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control font"
                                    value="{{ $branchId ? $branches->firstWhere('id', $branchId)?->name : '' }}" disabled>
                            ...
                            <div class="text-muted small mt-1">
                                {{ trans('hr.night_shift_hint') ?? 'للوردية الليلية: إذا كانت نهاية الدوام بعد منتصف الليل (مثال 22:00 → 06:00) أدخل وقت الانصراف 06:00 وسيُحسب تلقائياً لليوم التالي.' }}
                            </div>
