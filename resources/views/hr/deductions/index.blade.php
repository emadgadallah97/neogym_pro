...
@section('title')
    {{ trans('hr.deductions') ?? 'الخصومات والجزاءات' }} | {{ trans('main_trans.title') }}
@endsection
    ...
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font">
                ...
                {{ trans('hr.deductions') ?? 'الخصومات والجزاءات' }}
            </h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="{{ route('hr.index') }}">{{ trans('hr.title') ?? 'الموارد البشرية' }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('hr.deductions') ?? 'الخصومات والجزاءات' }}</li>
                </ol>
            </div>
        </div>
    </div>
...
{{-- Filters --}}
<form method="GET" action="{{ route('deductions.index') }}" class="card border-0 shadow-sm mb-3">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-12 d-flex justify-content-between mt-2">
                ...
                @can('hr_deductions_create')
                <button type="button" class="btn btn-success font" id="btnOpenAdd">
                    <i class="ri-add-line me-1"></i> {{ trans('hr.add_deduction') ?? 'إضافة' }}
                </button>
                @endcan
            </div>
...
<div class="card border-0 shadow-sm">
    <div class="card-header">
        <h5 class="card-title mb-0 font">{{ trans('hr.deductions_list') ?? 'قائمة الخصومات والجزاءات' }}</h5>
    </div>

    <div class="card-body">
        <table id="deductionsTable" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
            <thead>
                <tr>
                    <th>#</th>
                ...
                @foreach($rows as $r)
                    ...
                    <tr id="row-{{ $r->id }}" data-id="{{ $r->id }}">
                        ...
                        <td class="text-center">
                            @if($r->type === 'deduction')
                                <span class="badge bg-secondary">{{ trans('hr.type_deduction') ?? 'خصم' }}</span>
                            @else
                                <span class="badge bg-dark">{{ trans('hr.type_penalty') ?? 'جزاء' }}</span>
                            @endif
                        </td>
                        <td class="text-center">{{ number_format((float)$r->amount,2) }}</td>
...
{{-- Add/Edit Modal --}}
<div id="deductionModal" class="modal fade" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 overflow-hidden">
            <div class="modal-header p-3">
                <h4 class="card-title mb-0 font" id="modalTitle">{{ trans('hr.add_deduction') ?? 'إضافة خصم/جزاء' }}</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>

            <div class="modal-body">
                <div id="appliedLockHint" class="alert alert-warning d-none font">
                    {{ trans('hr.deduction_applied_locked') ?? 'هذا السجل تم تطبيقه على كشف راتب ولا يمكن تعديله/حذفه.' }}
                </div>
                ...
                <form id="deductionForm" autocomplete="off">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <label class="form-label font">{{ trans('hr.type') ?? 'النوع' }}</label>
                            <select name="type" id="form_type" class="form-select font" required>
                                <option value="deduction">{{ trans('hr.type_deduction') ?? 'خصم' }}</option>
                                <option value="penalty">{{ trans('hr.type_penalty') ?? 'جزاء' }}</option>
                            </select>
                            <div class="invalid-feedback" id="form_type_error"></div>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label font">{{ trans('hr.reason') ?? 'السبب' }}</label>
                            <input type="text" name="reason" id="form_reason" class="form-control font" required>
                            <div class="invalid-feedback" id="form_reason_error"></div>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label font">{{ trans('hr.amount') ?? 'القيمة' }}</label>
                            <input type="number" step="0.01" min="0.01" name="amount" id="form_amount" class="form-control font" required>
                            <div class="invalid-feedback" id="form_amount_error"></div>
                        </div>
...
$(document).ready(function () {
    ...
    // permissions for JS
    var canEdit    = {{ auth()->user()->can('hr_deductions_edit')    ? 'true' : 'false' }};
    var canApprove = {{ auth()->user()->can('hr_deductions_approve') ? 'true' : 'false' }};
    var canDelete  = {{ auth()->user()->can('hr_deductions_delete')  ? 'true' : 'false' }};
    ...
    var table = $('#deductionsTable').DataTable({
        responsive: true,
    ...
    function initSelect2(){
        ...
        // داخل المودال
        $('#form_branch_id').select2({ width:'100%', dropdownParent: $('#deductionModal') });
        $('#form_employee_id').select2({ width:'100%', dropdownParent: $('#deductionModal') });
        $('#form_type').select2({ width:'100%', dropdownParent: $('#deductionModal') });
    }
    initSelect2();

    // reload filter employees by branch (اختياري)
    $('#filter_branch_id').on('change', function(){
        ...
        $.get('{{ route('deductions.employees.byBranch') }}', { branch_id: branchId }, function(res){
            if (res.success) {
    ...
    function clearErrors(){
        ['branch_id','employee_id','type','reason','amount','date','applied_month','notes'].forEach(function(f){
    ...
    function typeBadge(type){
        if (type === 'deduction') return '<span class="badge bg-secondary">{{ trans('hr.type_deduction') ?? 'خصم' }}</span>';
        return '<span class="badge bg-dark">{{ trans('hr.type_penalty') ?? 'جزاء' }}</span>';
    }
    ...
    $('#form_date').on('change', function(){
        ...
        $('#form_applied_month').val(ym);
    });

    // Open Add
    $('#btnOpenAdd').on('click', function(){
        clearErrors();
        $('#modalTitle').text('{{ trans('hr.add_deduction') ?? 'إضافة خصم/جزاء' }}');
        $('#form_id').val('');
        ...
        $('#form_type').val('deduction').trigger('change');
    ...
    $(document).on('click', '.btn-edit', function(){
        ...
        $.get('{{ url('deductions') }}/' + id, function(res){
            if(!res.success){
            ...
            $('#modalTitle').text('{{ trans('hr.edit_deduction') ?? 'تعديل خصم/جزاء' }}');
            $('#form_id').val(d.id);

            $('#form_branch_id').val(d.branch_id).trigger('change');
            loadModalEmployees(d.branch_id, d.employee_id);

            $('#form_type').val(d.type).trigger('change');
            $('#form_reason').val(d.reason);
            $('#form_amount').val(d.amount);
    ...
    $('#deductionForm').on('submit', function(e){
        ...
        var id = $('#form_id').val();
        var url = id ? ('{{ url('deductions') }}/' + id) : '{{ route('deductions.store') }}';
        ...
        $.ajax({
            ...
            success: function(res){
                ...
                var rowData = [
                    ...
                    d.branch_name,
                    '<div class="text-center">' + typeBadge(d.type) + '</div>',
                    '<div class="text-center">' + d.amount + '</div>',
    ...
    $(document).on('click', '.btn-approve', function(){
            ...
            $.ajax({
                ...
                success: function(res){
                    ...
                    if(row.any()){
                        ...
                        rowData[7] = '<div class="text-center">' + statusBadge(d.status) + '</div>';
                        rowData[9] = '<div class="text-center">' + actionsHtml(d) + '</div>';
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                text: '{{ trans('hr.deduction_approve_confirm_msg') ?? 'هل تريد اعتماد هذا الخصم/الجزاء؟' }}',
                icon: 'question',
