...
@php
    ...
    $chips = $chips ?? [];
    $kpis  = $kpis ?? [];
    $group = $group ?? ['rows'=>[],'group_by'=>'payment_method'];
    $rows  = $rows ?? collect();

    $meta['title'] = $meta['title'] ?? (__('reports.payments_report_title') ?? 'تقرير المدفوعات');
    ...
    $badge = function($status){
        ...
        $map = [
            'paid' => ['success', __('reports.pay_status_paid') ?? 'paid'],
            'pending' => ['warning', __('reports.pay_status_pending') ?? 'pending'],
            'failed' => ['danger', __('reports.pay_status_failed') ?? 'failed'],
            'refunded' => ['info', __('reports.pay_status_refunded') ?? 'refunded'],
        ];
...
<div class="container">
    ...
    <div class="main-header">
        ...
        <div class="header-right">
            ...
            <div>{{ __('reports.pay_kpi_net_collected') ?? 'صافي التحصيل' }}: {{ (float)($kpis['net_collected'] ?? 0) }}</div>
        </div>
    </div>
    ...
    <div class="summary-cards">
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pay_kpi_paid') ?? 'مدفوع' }}</div>
            <div class="summary-value">{{ (float)($kpis['paid_sum'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pay_kpi_refunded') ?? 'مسترد' }}</div>
            <div class="summary-value">{{ (float)($kpis['refunded_sum'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pay_kpi_pending_failed') ?? 'معلق/فشل' }}</div>
            <div class="summary-value">{{ (float)($kpis['pending_sum'] ?? 0) }} / {{ (float)($kpis['failed_sum'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pay_kpi_net_collected') ?? 'صافي التحصيل' }}</div>
            <div class="summary-value">{{ (float)($kpis['net_collected'] ?? 0) }}</div>
        </div>
    </div>

    <h3 class="section-title">{{ __('reports.pay_group_title') ?? 'ملخص التجميع' }}</h3>
    <div class="section-subtitle">{{ __('reports.pay_group_hint') ?? 'حسب خيار التجميع' }}</div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <thead>
            <tr>
                <th>{{ __('reports.pay_group_col_name') ?? 'البند' }}</th>
            ...
            @forelse(($group['rows'] ?? []) as $g)
                <tr>
                    ...
                    <td class="text-center">{{ (int)($g['payments_count'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['paid_sum'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['refunded_sum'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['pending_sum'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['failed_sum'] ?? 0) }}</td>
                </tr>
            @empty
    ...
    <h3 class="section-title">{{ __('reports.pay_table_title') ?? 'تفاصيل عمليات الدفع' }}</h3>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tr>
                <th>#</th>
                <th>{{ __('reports.pay_col_date') ?? 'التاريخ' }}</th>
            ...
            @forelse($rows as $i => $r)
                @php
                    ...
                    $dt = $r->paid_at ? \Carbon\Carbon::parse($r->paid_at)->format('Y-m-d H:i') : ($r->created_at ? \Carbon\Carbon::parse($r->created_at)->format('Y-m-d H:i') : '-');
                ...
                <tr>
                    ...
                    <td class="text-center">{{ (float)($r->amount ?? 0) }}</td>
