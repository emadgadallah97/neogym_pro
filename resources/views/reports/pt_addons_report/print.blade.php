...
@php
    ...
    $chips = $chips ?? [];
    $kpis  = $kpis ?? [];
    $group = $group ?? ['rows'=>[],'group_by'=>'trainer'];
    $rows  = $rows ?? collect();

    $meta['title'] = $meta['title'] ?? (__('reports.pt_addons_report_title') ?? 'تقرير PT Add-ons');
...
<div class="container">
    ...
    <div class="summary-cards">
        <div class="summary-card">
            ...
            <div class="summary-value">{{ (int)($kpis['addons_count'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pt_kpi_total_amount') ?? 'إجمالي القيمة' }}</div>
            <div class="summary-value">{{ (float)($kpis['total_amount_sum'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.pt_kpi_sessions_total') ?? 'الحصص (إجمالي/مستخدم/متبقي)' }}</div>
            <div class="summary-value">
                {{ (int)($kpis['sessions_total_sum'] ?? 0) }} /
                {{ (int)($kpis['sessions_used_sum'] ?? 0) }} /
                {{ (int)($kpis['sessions_remaining_sum'] ?? 0) }}
            </div>
        </div>
        <div class="summary-card">
    ...
    <div class="table-wrap">
        <table>
            ...
            <thead>
            <tr>
                <th>{{ __('reports.pt_group_col_name') ?? 'البند' }}</th>
                <th>{{ __('reports.pt_group_col_count') ?? 'عدد' }}</th>
            ...
            @forelse(($group['rows'] ?? []) as $g)
                <tr>
                    ...
                    <td class="text-center">{{ (int)($g['addons_count'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['total_amount_sum'] ?? 0) }}</td>
        ...
        <table>
            ...
            @forelse($rows as $i => $r)
                @php
                    ...
                    $date = $r->created_at ? \Carbon\Carbon::parse($r->created_at)->format('Y-m-d H:i') : '-';
            ...
            @empty
                <tr>
                    <td colspan="12" class="text-center" style="padding:18px;color:#999">{{ __('reports.no_results') ?? 'لا توجد نتائج' }}</td>
                </tr>
            @endforelse
            </tbody>
        </table>
