...
@php
    ...
    $chips = $chips ?? [];
    $kpis  = $kpis ?? [];
    $group = $group ?? ['rows'=>[],'group_by'=>'branch'];
    $rows  = $rows ?? collect();

    $meta['title'] = $meta['title'] ?? (__('reports.sales_report_title') ?? 'تقرير المبيعات');
    ...
    $badge = function($status){
        ...
        $map = [
            'active' => ['success', __('reports.sub_status_active') ?? 'active'],
            'expired' => ['secondary', __('reports.sub_status_expired') ?? 'expired'],
            'frozen' => ['warning', __('reports.sub_status_frozen') ?? 'frozen'],
            'cancelled' => ['danger', __('reports.sub_status_cancelled') ?? 'cancelled'],
            'pending' => ['info', __('reports.sub_status_pending') ?? 'pending'],
        ];

        if (isset($map[$s])) {
            return '<span class="badge badge-'.$map[$s][0].'">'.$map[$s][1].'</span>';
        }
        return '<span class="badge badge-dark">'.e($s ?: '-').'</span>';
    };
@endphp
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .badge-secondary{background:#f3f4f6;color:#374151;border:1px solid #e5e7eb}
        .badge-warning{background:#fff7ed;color:#9a3412;border:1px solid #fed7aa}
        .badge-danger{background:#fee2e2;color:#991b1b;border:1px solid #fecaca}
        .badge-info{background:#ecfeff;color:#155e75;border:1px solid #a5f3fc}
        .badge-dark{background:#111827;color:#fff;border:1px solid #111827}
...
<div class="container">
    ...
    <div class="main-header">
        ...
        <div class="header-right">
            ...
            <div>{{ __('reports.sales_kpi_total_sales') ?? 'إجمالي المبيعات' }}: {{ (float)($kpis['total_sales'] ?? 0) }}</div>
            <div>{{ __('reports.sales_kpi_total_discount') ?? 'إجمالي الخصم' }}: {{ (float)($kpis['total_discount'] ?? 0) }}</div>
        </div>
    </div>
    ...
    <div class="summary-cards">
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.sales_kpi_total_sales') ?? 'إجمالي المبيعات' }}</div>
            <div class="summary-value">{{ (float)($kpis['total_sales'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.sales_kpi_subs_count') ?? 'عدد الاشتراكات' }}</div>
            <div class="summary-value">{{ (int)($kpis['subs_count'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.sales_kpi_avg_sale') ?? 'متوسط قيمة البيع' }}</div>
            <div class="summary-value">{{ (float)($kpis['avg_sale'] ?? 0) }}</div>
        </div>
        <div class="summary-card">
            <div class="summary-label">{{ __('reports.sales_kpi_pt_addons') ?? 'مبيعات PT' }}</div>
            <div class="summary-value">{{ (float)($kpis['pt_addons_sales'] ?? 0) }}</div>
        </div>
    </div>

    <h3 class="section-title">{{ __('reports.sales_group_title') ?? 'ملخص التجميع' }}</h3>
    <div class="section-subtitle">{{ __('reports.sales_group_hint') ?? 'حسب خيار التجميع' }}</div>
    ...
    <div class="table-wrap">
        <table>
            ...
            <thead>
            <tr>
                <th>{{ __('reports.sales_group_col_name') ?? 'البند' }}</th>
            ...
            @forelse(($group['rows'] ?? []) as $g)
                <tr>
                    ...
                    <td class="text-center">{{ (int)($g['subs_count'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['total_sales'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['total_discount'] ?? 0) }}</td>
                    <td class="text-center">{{ (float)($g['pt_addons_sales'] ?? 0) }}</td>
                </tr>
            @empty
    ...
    <h3 class="section-title">{{ __('reports.sales_table_title') ?? 'تفاصيل المبيعات (الاشتراكات)' }}</h3>
    ...
    <div class="table-wrap">
        <table>
            ...
            <tr>
                ...
                <th>{{ __('reports.sales_col_sale_date') ?? 'تاريخ البيع' }}</th>
            ...
            @forelse($rows as $i => $r)
                @php
                    ...
                    $saleDate = $r->created_at ? \Carbon\Carbon::parse($r->created_at)->format('Y-m-d H:i') : '-';

                    $memberName = trim((string)($r->member_name ?? ''));
                    $memberCode = trim((string)($r->member_code ?? ''));
                    if ($memberName === '' && !empty($r->member_id)) $memberName = '#'.$r->member_id;

                    $offerName = $nameJsonOrText($r->offer_name ?? null);
                    $couponName = $nameJsonOrText($r->coupon_name ?? null);
                    $couponCode = trim((string)($r->coupon_code ?? ''));
                @endphp
                <tr>
                    ...
                    {{-- Member: name + code --}}
                    <td class="wrap">
                        <span class="block fw-semibold">{{ $memberName ?: '-' }}</span>
                        <span class="block muted small">
                            {{ __('reports.member_code') ?? 'كود العضو' }}: {{ $memberCode ?: '-' }}
                        </span>
                    </td>

                    <td class="wrap">
                        <span class="block"><span class="muted small">{{ __('reports.sales_col_plan_code') ?? 'الكود' }}:</span> {{ $r->plan_code ?? '-' }}</span>
                        <span class="block"><span class="muted small">{{ __('reports.sales_col_plan') ?? 'الخطة' }}:</span> {{ $planName }}</span>
                        <span class="block muted small">{{ __('reports.sales_col_type') ?? 'النوع' }}: {{ $typeName }}</span>
                    </td>

                    <td class="text-center">{!! $badge($r->status ?? null) !!}</td>

                    <td class="wrap">
                        <span class="block"><span class="muted small">{{ __('reports.sales_col_total_discount') ?? 'الإجمالي' }}:</span> {{ (float)($r->total_discount ?? 0) }}</span>
                        <span class="block muted small">{{ __('reports.sales_col_offer_discount') ?? 'عرض' }}: {{ (float)($r->discount_offer_amount ?? 0) }}</span>
                        <span class="block muted small">{{ __('reports.sales_col_coupon_discount') ?? 'كوبون' }}: {{ (float)($r->discount_coupon_amount ?? 0) }}</span>
                    </td>

                    <td class="wrap">
                        <span class="block"><span class="muted small">{{ __('reports.sales_col_total_amount') ?? 'الإجمالي' }}:</span> {{ (float)($r->total_amount ?? 0) }}</span>
                        <span class="block muted small">{{ __('reports.sales_col_price_plan') ?? 'الخطة' }}: {{ (float)($r->price_plan ?? 0) }}</span>
                        <span class="block muted small">{{ __('reports.sales_col_price_pt_addons') ?? 'PT' }}: {{ (float)($r->price_pt_addons ?? 0) }}</span>
                    </td>

                    {{-- Offer/Coupon: show names + coupon code --}}
                    <td class="wrap text-center">
                        <span class="block muted small">
                            {{ __('reports.sales_col_offer') ?? 'عرض' }}:
                            {{ !empty($r->offer_id) ? ($offerName ?: ('#'.$r->offer_id)) : '-' }}
                        </span>
                        <span class="block muted small">
                            {{ __('reports.sales_col_coupon') ?? 'كوبون' }}:
                            @if(!empty($r->coupon_id))
                                {{ $couponName ?: ('#'.$r->coupon_id) }}{{ $couponCode ? ' ('.$couponCode.')' : '' }}
                            @else
                                -
                            @endif
                        </span>
                    ...
                    <td class="wrap text-center">{{ $r->sales_employee_name ?? '-' }}</td>
                </tr>
            @empty
                <tr>
