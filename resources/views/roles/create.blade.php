...
<div class="container-fluid py-4">
    ...
    @if ($errors->any())
    <div class="alert alert-danger alert-dismissible fade show shadow-sm">
        <i class="mdi mdi-alert-outline me-1"></i> {{ trans('main_trans.validation_error') }}
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="fw-bold text-info mb-1">
                <i class="mdi mdi-account-key-outline me-2"></i>
    ...
    <form action="{{ route('roles.store') }}" method="POST" class="bg-white rounded-4 shadow p-4">
        @csrf

        <div class="row mb-3">
            <div class="col-lg-6">
                <label class="form-label fw-bold">
                    <i class="mdi mdi-shield-key-outline me-1 text-info"></i>
        ...
        <div class="accordion accordion-flush" id="permissionAccordion">
            ...
            <div class="accordion-item border mb-3 rounded shadow-sm">
                <h2 class="accordion-header" id="heading{{ $loop->index }}">
                    <button class="accordion-button collapsed fw-bold text-dark bg-light d-flex justify-content-between align-items-center"
                        type="button" data-bs-toggle="collapse"
                        data-bs-target="#collapse{{ $loop->index }}" aria-expanded="false">
                        <span><i class="mdi mdi-folder-lock me-2 text-info"></i> {{ $category }}</span>
                        <span class="badge bg-primary rounded-pill count-badge" data-total="{{ count($permissions) }}">
                            0/{{ count($permissions) }}
                        </span>
                    </button>
                </h2>
                            ...
                            <div class="col-md-4 col-lg-3 mb-2 permission-item">
                                <div class="form-check form-switch">
                                    <input class="form-check-input perm-checkbox" type="checkbox"
                                        name="permission[]" value="{{ $perm->name }}">
    ...
    window.addEventListener('load', function() {
        ...
        document.getElementById('permissionSearch').addEventListener('input', function() {
            ...
            document.querySelectorAll('.permission-item').forEach(item => {
                item.classList.toggle('hide', !item.textContent.toLowerCase().includes(term));
            });
        });
