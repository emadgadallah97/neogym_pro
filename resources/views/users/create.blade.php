...
@section('title', trans('users.title_create'))

@section('content')
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">{{ trans('users.title_create') }}</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="{{ route('users.index') }}">{{ trans('users.title') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('users.title_create') }}</li>
                </ol>
            </div>
        </div>
    </div>
        ...
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white border-bottom py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold">
                    <i class="ri-user-add-line me-2 text-primary fs-20 align-middle"></i>
                ...
                <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name') }}" placeholder="{{ trans('users.name') }}" required>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">{{ trans('users.password') }} <span class="text-danger">*</span></label>
                            <input type="password" name="password" class="form-control" placeholder="••••••••" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">{{ trans('users.password_confirmation') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="Status" class="form-select select2" required>
                                <option value="enabled" {{ old('Status', 'enabled') == 'enabled' ? 'selected' : '' }}>{{ trans('users.status_active') }}</option>
                                <option value="disabled" {{ old('Status') == 'disabled' ? 'selected' : '' }}>{{ trans('users.status_inactive') }}</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="branch_id" class="form-select select2" required>
                                <option value="" disabled selected>{{ trans('users.select_branch') }}</option>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-3">
                                    <div class="form-check card-radio p-0">
                                        <input class="form-check-input" type="checkbox" name="roles_name[]" value="{{ $role }}" id="role_{{ $role }}">
